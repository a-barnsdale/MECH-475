...
* Fixed-wing concepts with added VTOL capabilities, as well as all-electric multi-rotor concepts can be considered.
...
<a id='mission'></a>
#### Mission
This category contains all mission segments ordered from start to end of the trip (i.e. without refueling or recharging batteries). Each group can arbitrarily **name** each mission segment but **mission types must match the types available in the tool**. 

Mission segments are categorized as fixed-wing (FW) and/or VTOL segments. Below you can find the list of mission segments and respective parameters available in this tool:
##### **Taxi**
  ```json
    {
        "name": "A",
...
##### ***aircraft.rotary_wing* or *aircraft.vtol_fixed_wing* type components**
```json
{
    "name": "Aircraft",
...
```
 * This component must be present in any JSON file
 * If your configuration uses a fixed-wing to generate lift during forward flight, then choose the *aircraft.vtol_fixed_wing* type; if your configuration relies on rotors to generate lift during forward flight, then choose the *aircraft.rotary_wing* type.
...
<a id='theory_contents'></a>
## **Contents**
1. [Energy mass](#energy_mass)
    1. [Fixed-wing](#fixedwing)
        1. [Taxi](#taxi_theory)
        2. [Cruise](#cruise)
            1. [Conventional](#conventional)
            2. [Hybrid](#hybrid)
            3. [Battery recharging](#recharge)
        3. [Hold/Endurance/Loiter](#loiter)
        4. [Landing](#landing)
    2. [VTOL](#vtol)
        1. [Vertical climb](#verticalclimb)
        2. [Vertical descent](#verticaldescent)
        3. [Hover](#hover_theory)
    3. [VTOL & FW](vtol&fw)
        1. [Climb](#climb)
...
3. [Emissions](#emissions)
    1. [Batteries](#batteries)
        1. [Production](#bat_prod)
        2. [Operation](#bat_op)
    2. [Fuel](#fuel)
        1. [Production](#fuel_prod)
        2. [Operation](#fuel_op)
    3. [Hydrogen fuel cell](#h2_fc)
        1. [Production](#h2_prod)
        2. [Operation](#h2_op)


<a id='energy_mass'></a>
### Energy mass
In this section, we report the calculations of energy mass required to perform fixed-wing and VTOL segments. 

For *fuel-based* propulsion systems, the fuel mass fraction in segment $j$ - labelled as $\text{mf}_j$ - is given by, 
$$ \text{mf}_j = \frac{M_{f}}{M_{i}} = \frac{M_i - m_{\text{fuel},j}}{M_i} = 1 - \frac{m_{\text{fuel},j}}{M_i},$$
where $m_{\text{fuel},j}$ is the fuel mass for segment $j$, $M_i$ and $M_f$ are the aircraft mass at the start and end of the mission segment, respectively. 

If $\text{mf}_j$ is known for segment $j$, the fuel mass required for segment $j$ is calculated as,
$$ m_{\text{fuel},j} = M_i(1-\text{mf}_j).$$

For configurations using biofuels as energy source, we convert any empirical data on $\text{mf}_j$ using the ratio between specific energies of biofuels and jet-fuel,
$$ E_{\text{jet-fuel}} = E_{\text{biofuel}} \Leftrightarrow \left(m\times e^*\right)_{\text{jet-fuel}} = \left(m\times e^*\right)_{\text{biofuel}} \Leftrightarrow m_{\text{biofuel}} = m_{\text{fuel}}\times \frac{e^*_{\text{jet-fuel}}}{e^*_{\text{biofuel}}}.$$

This approximation considers the energy requirements for both jet-fuel and bio-fuel-based systems to be the same. Another potential correction to be added in future versions of the tool is the increase in efficiency of current turbine engines.

The aircraft mass at the end of segment $j$ is,
$$ M_f = M_i - m_{\text{fuel},j}.$$

The total fuel mass is the sum of contributions from each segment, 
$$ m_{\text{fuel}} = \sum_{j = 1}^N m_{\text{fuel},j}.$$

For *electric-based* propulsion systems, the mass of energy source required to satisfy each mission segment is calculated using the following methodology:
$$ \text{Segment power} \rightarrow \text{Segment energy} \rightarrow \text{Energy mass}.$$ 
1. At segment $i$, the conversion of power, $P_i$, to energy, $E_i$, depends on the time of segment, $t_i$, as follows,
$$ E_i = P_i \times t_i.$$
2. Energy is then converted to mass as follows,
$$ m_i = \frac{E_i}{e^*_{\text{spec}}\times \eta_p \times f_{\text{usable}}},$$ 
where $e^*_{\text{spec}}$ is the specific energy of the energy source, $\eta_p$ is the efficiency of the energy network, and $f_{\text{usable}}$ is the fraction of usable energy.

3. The mass of energy source is updated throughout the mission profile,
$$ m = m_{i-1} + m_i.$$

### **Fixed-wing**
<a id='taxi_theory'></a>
#### **Taxi**
This segment is currently only valid for powerplants using fuel as energy source. This includes the more traditional jet-fuel and eco-friendly biofuels. 

The fuel mass spent while taxiing, $m_{\text{fuel,taxi}}$ is calculated as,
$$ m_{\text{fuel,taxi}} = m_{\text{fuel}}\times \frac{e^*_{\text{jet-fuel}}}{e^*_{\text{fuel}}} = M_i(1-0.9725)\frac{e^*_{\text{jet-fuel}}}{e^*_{\text{fuel}}}.$$ 

#### **Climb**
For *fuel-based* propulsion systems, the vehicle mass fractions are given as a function of the Mach number at the start of the next segment, $M$,
$$ \text{mf}_{\text{climb}} = \begin{cases} 1 - 0.04M & M < 1 \\
                                        0.96-0.03(M-1) & M > 1.\end{cases}
$$ 
...
### **VTOL**
#### **Correction for different rotor sizes**
The mass calculations for vertical flight segments are a function of each rotor's thrust. In the course, we assume that all rotors are geometrically identical and generate the same amount of thrust. However, this approximation is not valid in the following two scenarios:
1. Configurations with rotors of different sizes,
2. Configurations with geometrically identical rotors but unequally positioned with respect to the aircraft's centre of gravity.

In this tool, we account for scenario 1 by computing the thrust generated by each rotor under the assumption the disk loading of each rotor, $T/A$, is the same. 

Consider the example of a VTOL aircraft with four rotors of areas $A_i$ and thrust $T_i$ shown in the figure below.

![mission-profile](figures/vertical_forces.png)

The balance of vertical forces (assuming drag to be negligible) is given by,
$$ W = \sum_{i=1}^{NR} T_i,$$
where NR is the number of rotors. If we use rotor 1 as reference, the ratio of areas between each rotor $i$ and the rotor of reference yields,
$$ n_i = \frac{A_i}{A_1}.$$

For disk loading to be constant between any rotor $i$ and the reference one, their ratio of thrust forces is given by,
$$ \left(\frac{T}{A}\right)_{i} = \left(\frac{T}{A}\right)_{1} \Leftrightarrow T_i = n_iT_1,\quad i \neq 1.$$

Substituting this result in the balance of forces, one obtains,
$$ T_1 = \frac{W}{1 + n_1 + n_2 + n_3}.$$ 

**NOTE:** It is the designer's responsability to assure the aircraft remains balanced during vertical climb with an appropriate choice of the c.g.'s position. Future work involves including moment calculations using the c.g. position of the aircraft according to the user's input. This modification will address scenario 2 mentioned above.

#### **Coaxial rotors**
For co-axial rotors, the induced power is increased by a factor of $\sqrt{2}$ with respect to the total induced power of two isolated rotors. Using the induced power of a single rotor as reference, the power induced of coaxial rotors yields,
$$ \frac{P_{i,\text{coaxial}}}{2\times P_{i,\text{1 rotor}}} = \sqrt{2} \Leftrightarrow P_{i,\text{coaxial}} = 2\sqrt{2} P_{i,\text{1 rotor}}.$$ 

The additional set of rotors increases the profile power by two with respect to an isolated rotor case,
$$ P_{\text{profile,coaxial}} = 2\times P_{\text{profile,isolated}}$$ 
#### **Ducted fans/rotors**
The reduction in induced power requirements of a ducted fan, $(P_i)_{\text{fan}}$, with respect to an open rotor is given by,
$$ (P_i)_{\text{fan}} = \frac{1}{\sqrt{2a_w}}P_{i,\text{1 rotor}},$$ 
where $a_w$ is a contraction factor (assumed to be 1).

### Segments
#### Hover
Consider an aircraft with rotors of $m$ different sizes. The total power required consists of **induced power**, $P_i$ and **profile power**, $P_p$,
$$ P_i = \sum_{j = 1}^m k_i \times N_{r,j} \times T_j \times \sqrt{\frac{T_j}{2\rho A_j}},$$
$$ P_p = \sum_{j = 1}^m A_j \times \rho \times N_{r,j} \times V_{\text{tip}}^3 \times \frac{\sigma \times C_{d0}}{8}.$$ 

#### Climb
The total power required consists of the sum of **induced power**, $P_i$, **blade profile power**, $P_b$, **parasitic power**, $P_p$, and **climb power**, $P_c$,
$$ P_i = k_i\times \frac{W^2}{2\times \rho \times N_r \times A_r \times V_{\infty}}, $$
$$ P_b = A_r \times N_r \times \rho \times \sigma \times C_{d_0} \times (1 + 4.65 \times \mu^2) \times \frac{(n \times R)^3}{8},$$
$$P_c = W V_{\infty} \sin \gamma_{\text{cl}}, $$
$$ P_p = \frac{1}{2}\rho V_{\infty}^3f.$$   

The advance ratio, $\mu$, is given by $$ \mu = \frac{V_{\infty}\cos\alpha}{nR},$$ where the blade angle-of-attack, $\alpha$ is approximated as, $$ \alpha = \gamma_{\text{cl}} + \frac{1}{L/D}.$$ 
The term $n\times R$ was not susbtituted by the tip velocity since, in forward flight conditions, different sections of the rotors experience different tip velocities due to the added/subtracted component of the forward velocity. 

One can conclude that the power required for horizontal level flight assumes $P_c = 0$. 

#### Recharging cruise 
In a recharging network, the rate of fuel burnt is split into an $x$ portion used for recharging the bateries, and $(1-x)$ used as propulsive energy,
$$ \dot{W}_{\text{fuel}} = \dot{W}_{\text{prop}} + x\dot{W}_{\text{fuel}}$$ 
...
#### **Drag**
The drag force experienced by the aircraft is the sum of three contributions:
* **Viscous drag**: the air in contact with the aircraft will develop friction forces opposite to the motion. 
* **Pressure drag**: consists of three contributions,
    * **Drag due to flow separations**,
    * **Lift-induced drag**,
    * **Wave drag**
