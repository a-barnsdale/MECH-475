...
% If there's a load step segment, check if aircraft mass is higher than initial mass. If so, it is defined as new MTOM.
for i = 1 : length(mission.segments)
    if strcmp(mission.segments{i}.type, 'load_step') 
        if mission.segments{i}.weight_final > mass_to
            vehicle.mass = mission.segments{i}.weight_final;
        end
    end
end
...
if is_type(source, 'energy.fuel')
    ...
    if is_type(aircraft, 'aircraft.rotary_wing')
        ...
        m_fuel = total_power_climb * segment.time / network_efficiency(network, 'no_type') / databases.source.specific_energy{fuel_type_id,1};
        vehicle.components{network_ids(source_id)}.mass = source{1}.mass + m_fuel;
        vehicle.mass = vehicle.mass - m_fuel;
        
    else % fuel-based climb segment for fixed-wing aircraft
